...
- [x] 1.1 Escribir `scripts/ejemplo_mca_famd.py` (stdlib pura, docstring con qué hace y cómo
...
- [x] 1.2 Añadir el **montaje de la rara**: Endulzante {Azúcar 4, Nada 3, Stevia 1}, con la
...
- [x] 1.3 Añadir las **suplementarias**: Nada y Stevia proyectadas sobre el MCA base por
      ...
      **Hecho cuando:** Stevia en (0,541, 1,414, −1,307) y Nada en (0,691, 0,471, 0,796)
      ...
      activa.
- [x] 1.4 Añadir el **FAMD** de Bebida, Horario, tazas y sueño: varianzas por columna,
...
- [x] 1.5 Poner los `assert` de autocomprobación del diseño D2 (inercia, transición para los
...
- [x] 1.6 Emitir `src/sessions/s06/data/ejemplo.js` con la cabecera «generated — do not
...
- [x] 2.1 Añadir a `scripts/clean_salon.py` el paso 10, `famd()`, con la convención de pesos
...
- [x] 2.2 Añadir el segundo montaje `sinRaras`: las categorías con n_j = 1 (encontradas por
      ...
      **Hecho cuando:** el script imprime qué categorías quedaron como suplementarias
      ...
      en los dos montajes.
- [x] 2.3 Añadir, solo al montaje activo, las **marcas** como suplementarias: por cada
...
- [x] 2.4 Emitir `FAMD` en `src/data/salon_limpio.js` con su comentario de cabecera, y
      regenerar el archivo.
      **Hecho cuando:** `scripts/clean_salon.py`, corrido desde el `.venv`, lo escribe, `PCA` y
...
- [x] 3.1 Añadir `comprobar_famd` a `scripts/check_salon.py` con las cinco comprobaciones
      del diseño D4, corriendo sobre los dos montajes, sin importar nada fuera de stdlib.
      **Hecho cuando:** `scripts/check_salon.py` pasa sin instalar nada,
      y falla con código 1 si se altera a mano un valor propio, un r², un baricentro o una
      contribución del archivo.
- [x] 3.2 Reescribir la lista de figuras de `scripts/check_figuras.mjs` para que importe
...
- [x] 4.1 Reescribir `src/sessions/s06/meta.js`: cuatro bloques —entrada 0–78, bloque 1
...
- [x] 4.2 Quitar `bloque-3` de la sesión 6 en `src/sessions/registry.js` y borrar
...
- [x] 4.3 Corregir el `Purpose` de `openspec/specs/sesion-06-estructura/spec.md` («cinco
      bloques» → «cuatro»).
      **Hecho cuando:** el Purpose describe la sesión de cuatro bloques.

## 5. Bloque 1 · MCA y categorías raras

- [x] 5.1 Copiar a `s06/figures/shared.js` los helpers de fórmulas de
...
- [x] 5.2 Dibujar en `block1.js` los tres mapas factoriales —base, con la rara activa, con
...
- [x] 5.3 Escribir `Block1.jsx`, partes 1 y 2 de la guía en el orden del diseño D6: el
...
- [x] 5.4 Escribir en `Block1.jsx` las partes 3 y 4: el montaje de la rara con su portador,
...
- [x] 5.5 Cerrar `Block1.jsx` con la parte 5: cuatro `Cards` —baricentro, inercia,
...
- [x] 6.1 Escribir en `s06/figures/block2.js` las figuras de fórmulas del bloque 2 (los dos
...
- [x] 6.2 Dibujar en `block2.js` los tres gráficos del FAMD del ejemplo —mapa de individuos
...
- [x] 6.3 Escribir `Block2.jsx`, partes 6 y 7 en el orden del diseño D6: el problema antes
      ...
      el individuo que más aporta y la identidad por baricentros; las advertencias y las
      alternativas, sin nombrar software ni lenguajes de programación.
      **Hecho cuando:** todos los elementos de `specs/sesion-06-famd` hasta las
      ...
      no encuentra ninguna cifra tecleada.
- [x] 6.4 Escribir en `Block2.jsx` la parte 8 y la síntesis: los tres gráficos con su
      gramática de lectura, el orden de lectura y qué hacer en conjuntos grandes, y la
      `Idea` final de que los tres métodos son el mismo algoritmo con distintos escalados.
      **Hecho cuando:** los elementos restantes de la spec están y el bloque se recorre en
      el navegador sin error de consola.
- [x] 6.5 Quitar de `Block2.jsx` el llamado de referencia en código y la frase que nombra
      el software, y decir en palabras que el método viene en los programas estadísticos
      habituales; el público no es técnico y el material no depende de un lenguaje.
      **Hecho cuando:** `grep -rni "python\|factominer\|prince\| en R\b" src/sessions/s06/blocks`
      solo devuelve la variable del formulario («usa Python») en la entrada y en el cierre,
      y `pnpm build` compila.

## 7. Cierre · El salón entero

- [x] 7.1 Dibujar en `s06/figures/closing.js`, desde `FAMD` de `salon_limpio.js` y con `id`
...
- [x] 7.2 Escribir `Closing.jsx` en el orden del diseño D7 hasta el filtro: qué entra (las
...
- [x] 7.3 Escribir la auditoría de las categorías de una persona: cuáles son (desde el
...
- [x] 7.4 Escribir la proyección de las marcas: una suplementaria por variable con celdas
...
- [x] 7.5 Escribir el ticket de salida con la consigna de la spec y sus respuestas que
...
- [x] 8.1 Borrar `Pendiente` de `src/components/content/index.jsx`, sus reglas de
...
- [x] 8.2 Actualizar `README.md`: el script nuevo entre los de stdlib pura, `sNN/data/` en
...
- [x] 8.3 Comprobar que la entrada sigue diciendo la verdad: «los tres bloques que vienen
...
- [x] 9.1 Ejecutar la verificación completa: `pnpm build`, los verificadores
      `scripts/check_salon.py`, `scripts/check_pca.py` y `scripts/ejemplo_mca_famd.py`, este
      dos veces con `diff`, y `node scripts/check_figuras.mjs`.
      **Hecho cuando:** los cinco pasan y `pnpm build` muestra cuatro chunks para la sesión 6
      más el chunk compartido de `ejemplo.js`.
- [x] 9.2 Recorrer los cuatro bloques de la sesión 6 con `pnpm preview` a ancho completo y
...
- [x] 9.3 Recorrer el índice y las otras cinco sesiones: ocho filas, la sesión 6 con cuatro
...
- [x] 9.4 Desplegar en local y compartir la URL para la comprobación manual.
