...
import { TABLA, CHI2, CA } from '../data/lluvia.js';
...
/* Block 1 of session 7 · the simple correspondence analysis of the rain table. Every id
   ...
   today, columns the sky tomorrow. */
...
export function fPerfiles() {
  ...
  [parte, y] = seccion(y, 'EL PERFIL DE UNA FILA', 'la fila dividida por su suma: es la probabilidad condicional de la entrada, y es un punto con tantas coordenadas como columnas · c_j es el perfil promedio: la columna sobre el total, el centroide',
    ...
    `en el ejemplo, «hoy ${f0.nivel}»: perfil (${f0.perfil.map(num).join(', ')}), masa ${f0.n}/${TABLA.n} = ${num(f0.masa)}. El centroide es el perfil promedio, el margen de columna sobre el total: c = (${CA.centroide.map(num).join(', ')}).`);
  s += parte;
  [parte, y] = seccion(y, 'LA DISTANCIA CHI-CUADRADO', 'entre dos perfiles: cada diferencia al cuadrado, dividida por el centroide de esa columna, c_j — las columnas raras pesan más',
    ...
    `en el ejemplo, entre «hoy ${CA.distancia.filas[0]}» y «hoy ${CA.distancia.filas[1]}»: d² = ${num(CA.distancia.d2)}, d = ${num(CA.distancia.d)}`);
  s += parte;
...
export function fTransicionCA() {
  ...
  [parte, y] = seccion(y, 'DE LAS COLUMNAS A LA FILA', 'una fila está en el promedio de las columnas, pesado por su perfil r_ij, dilatado por 1/√λ — y viceversa, con el perfil de columna c_ji = n_ij / n_·j: la columna repartida entre las filas. No es el c_j de la distancia, que es una sola cifra por columna',
    ...
    `en el ejemplo, «hoy ${t.fila}» en el eje ${t.eje}: ${t.sumandos.map(x => `${num(x.perfil)}·(${n3(x.coord)})`).join(' + ')} = ${n3(t.promedioPonderado)}, y ${n3(t.promedioPonderado)}/${num(t.raizLambda)} = ${n3(t.dilatado)}: su coordenada publicada es ${n3(t.coordPublicada)}`);
  s += parte;
  [parte, y] = seccion(y, 'CONTRIBUCIÓN Y COSENO CUADRADO', 'quién construyó el eje, y qué tan fiel es la posición de cada punto en él; las mismas dos herramientas para filas y para columnas',
    ...
    `en el ejemplo, «hoy ${f0.nivel}» en el eje 1: ${num(f0.masa)} · (${n3(f0.coord[0])})² / ${num(CA.autovalores[0])} = ${num(f0.ctr[0] / 100)}, es decir ${num(f0.ctr[0])} % del eje; cos² ${num(f0.cos2[0])}. El aporte promedio de una fila es 1/${CA.filas.length} = ${num(CA.aportePromedioFilas)} %.`);
  s += parte;
  return svg(W, y,
...
export function mapaCA() {
  ...
  CA.filas.forEach(f => {
    ...
    rotulos.push({ x: x + 10, y: y + 4, t: `hoy ${f.nivel} (${f.n})`, fill: COL_FILA, ancho: 140 });
  });
  CA.columnas.forEach(c => {
    const x = sx(c.coord[0]), y = sy(c.coord[1]);
    b += cuadrado(x, y, 12, COL_COL, { op: 0.95 });
    rotulos.push({ x: x + 10, y: y + 4, t: `mañana ${c.nivel} (${c.n})`, fill: COL_COL, ancho: 140 });
  });
  apilar(rotulos, 13).forEach(r => b += txt(r.x, r.y, r.t, { fs: 11, fill: r.fill }));

  b += txt(30, 28, 'EL CIELO DE HOY Y EL DE MAÑANA EN EL MISMO PLANO', { fs: 11, fill: C.ask, ls: 1.8 });
  b += dot(36, 56, 6, COL_FILA, { op: 0.9 });
  b += txt(50, 60, 'filas: el cielo de hoy, con sus días', { fs: 11, fill: C.ink2 });
  b += cuadrado(36, 74, 12, COL_COL);
  b += txt(50, 78, 'columnas: el cielo de mañana, con sus días', { fs: 11, fill: C.ink2 });
  wrap(`Una fila cerca de una columna: esa combinación tuvo más días de los esperados. Los dos ejes retienen ${num(CA.acumulado[1])} % de la inercia: con tres filas y tres columnas el plano lo dibuja todo, y el eje 1 solo lleva ${num(CA.porcentajes[0])} %.`, 30)
    .forEach((l, i) => b += txt(30, 104 + i * 15, l, { fs: 11, fill: C.ink3 }));
  ...
  return svg(W, H,
    'El plano de los dos ejes del análisis de correspondencias del cielo de hoy contra el cielo '
    + `de mañana: los ${CA.filas.length} estados de hoy como círculos y los ${CA.columnas.length} de mañana `
    + `como cuadrados, cada uno con su número de días. El eje 1 retiene ${num(CA.porcentajes[0])} por `
